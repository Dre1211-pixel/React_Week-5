...
function App() {
  ...
  const [isScreenLoading, setIsScreenLoading] = useState(false); // for loading spinner
  const [addingProductId, setAddingProductId] = useState(null); // for loading button-spinner


  const getCart = async () => {
    try {
      const res = await axios.get(`${BASE_URL}/v2/api/${API_PATH}/cart`); 
      setCart(res.data.data); // 注意回傳的資料結構是 res.data.data, 第一個 data 是 axios 的回傳物件，第二個 data 是我們的資料
    } catch (error) {
  ...
  const addCartItem = async (product_id, qty) => {
    setAddingProductId(product_id); // start loading button-spinner
    try {
      ...
      setAddingProductId(null); // stop loading button-spinner
    }
  }
            ...
            <div className="modal-content">
              ...
              <div className="modal-footer">
                <button
                  ...
                  disabled={addingProductId === tempProduct.id}
                >
                  加入購物車
                  {addingProductId === tempProduct.id && (
                    <ClipLoader size={14} color="#fff" />
                  )}
                </button>
              </div>
